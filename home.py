...
def layout_main(name):
    ...
    if a_sel is not None:
        ...
        fig = px.bar(df_sorted, x='Question', y='Percentage', text='percentage_text', width=1200, height=600)
        ...
        with st.form("單一問題分析"):
            ...
            # Add form components
            qs = list(df_1['Question'].unique())
            qs.sort()
            a_q = col1.selectbox('請選擇一個題目', qs)
            cc = list(df['班級'].unique())
            cc.sort()
            # cc.insert(0, 'All')
            a_c = col2.multiselect('選擇班級，可複選', cc, cc)
